...
const missions = [
  {
    ...
    prompt:
      "請扮演金融分析師。根據我提供的股票技術指標結果，包括 SMA20、SMA60、RSI、MACD、布林通道，請用教學語氣解釋目前趨勢、動能、波動狀態與可能風險。請不要給買賣建議，只能做分析。",
    colab:
  ...
  },
  {
    ...
    goal: "建立一個基礎策略回測模組，讓使用者理解交易訊號、報酬計算、績效比較與 look-ahead bias。",
    ...
    prompt:
      "請根據我的回測結果，包含策略累積報酬、Buy and Hold 累積報酬、年化報酬、年化波動度、Sharpe Ratio、最大回撤，幫我寫一段績效分析。請特別說明策略優勢、限制、可能過度配適問題，以及為什麼回測不代表未來報酬。",
    colab:
  ...
  },
  {
    ...
    prompt:
      "請扮演金融分析師。根據以下資料：技術指標摘要、策略回測績效、最大回撤、Sharpe Ratio、波動度，生成一份個股研究報告。報告需包含：一、資料摘要；二、技術面觀察；三、策略回測結果；四、風險與限制；五、教學重點。請避免任何直接買進、賣出或報酬保證用語。",
    colab:
  ...
  },
  {
    ...
    prompt:
      "請根據多檔股票的報酬率、波動度、最大回撤、技術指標分數與 AI Score，說明不同投資組合權重方法的差異。請比較等權重、反波動權重與最小變異權重的優缺點，並提醒使用者這不代表實際投資建議。",
    colab:
